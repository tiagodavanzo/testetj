$(document).ready(function() {
	...
	$('#subjects').DataTable({
		...
		responsive: true,
		ajax: {
			'url': 'source/datatable/subjects.php',
			'type': 'POST',
			'data': function ( d ) {
				d.action = 'getall';
			}
...
function Get(id)
{
	LoadTab(1, 'Atualizar')
	Loading()
	$.post('source/datatable/subjects.php', { 'action': 'get', 'id': id },
		function(data){
...
function Subject()
{
	if($("#frmSubject").valid()){
		...
		$.post('source/datatable/subjects.php', { 'action': action, 'id': id, 'name': name },
			function(data){
...
function Delete(id)
{
	...
	{
		$.post('source/datatable/subjects.php', { 'action': 'delete', 'id': id },
			function(data){
